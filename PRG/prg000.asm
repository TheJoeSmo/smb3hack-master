...
PRG000_XFCF:
	...
	LDX <entity_index	 ; Restore 'X' to its slot index value
...
Object_HandleConveyorBounceVel:
	...
	EOR <entity_lo_x_velocity,X	 ; Check if object is not going with conveyor
...
Object_HitGround:
	...
	STA <entity_lo_y,X	 ; Aligns to grid and adds 1
...
PRG000_C533:
	...
	STA <entity_lo_y,X	 ; Align to tile and apply slope

PRG000_C53D:
	LDA #$00
	STA <entity_lo_y_velocity,X	 ; Halt vertical movement
...
PRG000_C580:
	ORA <entity_collision_flags,X	; Set bit 0 or 1
	STA <entity_collision_flags,X	 ; Update entity_collision_flags
...
OWD_Not32PP:
	AND #$0f
	STA <var1	; var1 = object's tile-relative vertical position

	LDA <var16	; var16 right now is ObjTile_DetXLo
	AND #$0f
	STA <var16	; var16 = object's tile-relative horizontal position
...
PRG000_C5EC:
	...
	LSR A
	LSR A	
	STA <var2	 ; Upper 4 bits of slope (the "ceiling" slope height) -> var2
...
Object_WorldDetect_SlopeNoRev:
	...
	AND #$0f	
	STA <var2	 ; Lower 4 bits of slope (the "floor" slope height) -> var2
...
PRG000_C635:
	...
	LDA <var1		 ; A = value from Slope_ObjectVel_Effect
...
PRG000_C66C:
	ADD <entity_lo_y,X
	STA <entity_lo_y,X ; Update Y

	TYA		 ; A = $00/$FF, sign extension

	ADC <entity_hi_y,X
	STA <entity_hi_y,X ; Apply to Y Hi
...
Object_WorldDetect_FHChkRev:
	...
	LDA #16
	SUB <var1	 ; Calculate an inverted limit
	CMP <var2
...
Object_SlopeFlat_Correct:
	...
	AND #$f0
	SUB #$01
	STA <entity_lo_y,X	; Align object to one pixel above the tile
...
; Check to see if we need to activate a switch
	LDA entity_state, X 	; Held objects cannot activate the switch
	CMP #OBJSTATE_HELD
...
ObjectDo_QuickSand:
	...
	LDA #OBJSTATE_KILLED
	STA entity_state,X	; killed by quicksand
...
PRG000_C736:
	...
	LDA ObjTile_DetXLo
	STA <var16	 ; var16 = ObjTile_DetXLo
	...
	; var3/4 point to array of slope values for this quadrant
...
Object_ODTV_Cont:
	...
	; ObjTile_DetXHi = entity_hi_x (should be zero)
	LDA <entity_hi_x,X
	STA ObjTile_DetXHi
...
Podoboo_Splash:
	...
	LDA #$00
	STA <entity_lo_y_velocity,X	; Object's Y velocity is zeroed at impact of water
...
PRG000_C975:
	STX <entity_index	 ; Backup current object index -> entity_index
	...
	DEC entity_timer,X	 ; Otherwise, decrement it
...
PRG000_C983:
	...
	DEC entity_no_collision_timer,X	; Otherwise, decrement it
...
PRG000_C98B:
	...
	LDA entity_timer_tertirary,X

	BEQ PRG000_C997	 ; If this timer is already at zero, jump to PRG000_C997

	DEC entity_timer_tertirary,X	; Otherwise, decrement it
...
PRG000_C9B3:
	DEC entity_timer_secondary,X	 ; Every 4 ticks, decrement entity_timer_secondary
...
;
; Do whatever is required by the current entity_state value
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
Object_DoStateAction:
	LDA entity_state,X
	...
	LDA entity_type,X	; Get object ID
...
PRG000_CA7F:
	LDX <entity_index		 ; Restore X as the object slot index
...
PRG000_CA81:
	...
	LDA entity_state,X	 ; Get object state...
	CMP #OBJSTATE_INIT
...
ShelledDoNotAllignWithWall:
	...
	STA entity_var13, X 	; Store last Y velocity that isn't zero.
ShelledDoNotRememberLastYVel:
	...
	LDA <entity_lo_x_velocity,X	 ; Get X velocity
...
PRG000_CB30:
	...
	JSR Negate
PRG000_CB37: 
	STA <entity_lo_x_velocity,X	 ; Set as X velocity 
	...
	STA <entity_lo_y_velocity,X	 ; Set object Y velocity
...
Object_SetShakeAwakeTimer:
	...
	AND #$07
	STA entity_timer_tertirary,X	; Timer 4 = 0-7, based on timer3
...
PRG000_CBF8:
	...
	LDX <entity_index ; Restore 'X' as object index
...
PRG000_CC37:
	...
	LDY <entity_index	 ; Y = object slot index
 
	LDA <entity_sprite_lo_x,X 
	CMP entity_sprite_lo_x,Y 
	LDY #$00	 ; Y = 0 
...
PRG000_CC62:
	TYA 
	STA entity_flipped_animation,X	 ; Object turns around if it hit another object 
...
PRG000_CC75:
	...
	LDA ObjKickXvel,Y 
	STA <entity_lo_x_velocity,X	 ; Set X velocity based on which side of Player the object is on 
...
PRG000_CCAE:
	...
	; var15 = Object tile detect X Hi
	LDA ObjTile_DetXHi 
	STA <var15	 

	; var16 = Object tile detect X Lo
	LDA ObjTile_DetXLo 
	STA <var16	 
...
PRG000_CD36:
	...
	LDX <entity_index		 ; X = object slot index (our kicked object)
...
PRG000_CD46:
	...
	STA entity_flipped_animation,X	 ; Apply flip as appropriate
...
; $CDD2
Object_BumpBlocks:
	STA <var8	; Store detected tile -> var8
...
Player_KickObject:
	...
	JMP _Player_KickObject	 ; Will return automatically
Player_KickObjectDraw:
	PLA
	STA PAGE_A000
	JSR PRGROM_Change_A000
	JMP Object_DrawShelled	 ; Draw shelled object

Player_KickObjectNoDraw:
	; Return to the old bank and leave
	PLA
	STA PAGE_A000
	JMP PRGROM_Change_A000	


	; Assumes load from entity_type,X before entering!
...
ObjState_Killed:
	...
	LDA #$02
	STA entity_animation_frame,X	 ; Set frame to 2
...
; $D097
Object_Move:
	LDA <entity_lo_x_velocity,X	; Get Object's X velocity
	PHA		 	; Save it
	...
	ROR <entity_lo_x_velocity,X	; X velocity rotated right (divided by 2) and proper sign of bit 7 in place
...
Move_DetectBypass:
	...
	STA <entity_lo_y_velocity,X ; Otherwise, cut off velocity at OBJECT_MAXWATERUPWARD
...
PRG000_D0CA:
	...
	STA <entity_lo_y_velocity,X	; Apply gravity to object
	...
	STA <entity_lo_y_velocity,X	; Cap fall at max rate
...
Object_AboutFace:
	LDA <entity_lo_x_velocity,X	 
	JSR Negate
	STA <entity_lo_x_velocity,X	 ; Negate object's X velocity
...
Object_FlipFace:
	LDA entity_flipped_animation,X
	EOR #SPR_HFLIP
	STA entity_flipped_animation,X	 ; Flip left/right flag
...
PRG000_D1DB:
	...
	STA <var4	 ; Otherwise, store current state into var4
	...
	AND #$1f	 ; Cap 0-31
	ADC #-$4C	 
	STA <entity_lo_y_velocity,X	 ; Object Y Vel = -$2D to -$4C

	; Set object state to Killed
	LDA #OBJSTATE_KILLED
	STA entity_state,X

	LDA <player_lo_x_velocity	; Get Player X velocity
	...
	ADD <entity_lo_x_velocity,X	; Add the existing velocity to this (so object velocity is influenced by half Player velocity)
...
PRG000_D218:
	...
	; The way to think about this is we're going to reference entity_lo_y
	...
	; from that point.)  The judgement is based on player_lo_y, which is the
	...
	; |
	; G <-- top of 'Goomba' is entity_lo_y
	;
...
; End Here
	...
	BNE _not_shelled
	STA <var1
	LDA <player_lo_y		; This basically allows a player to grab a shelled shell as long as he's holding b
	SUB #1
	CMP <player_lo_y
	JMP _end_of_shell_check
_not_shelled:			; For non-shells, do normal stomp comparison
	LDA <entity_lo_y,X	; Get object's Y
	SUB <var2		; Subtract var2 (height above object considered "stompable" range)
	ROL <var1		; Stores the carry bit into var1 bit 0
	CMP <player_lo_y
...
_end_of_shell_check:
	...
	LSR <var1		; Restore the carry bit
	LDA <entity_hi_y,X	
	SBC #$00	 	; Apply the carry bit to the entity_hi_y as needed for the height subtraction

	PLP		 ; Restore CPU state (the comparison)

	SBC <player_hi_y		; Get the difference against the player_hi_y
	BMI Jmp_HoldKickOrHurtPlayer	 	; If negative (player_hi_y > entity_hi_y, Player is lower), jump to Jmp_HoldKickOrHurtPlayer (Object_HoldKickOrHurtPlayer)
...
ObjStomp_Rev:
	...
	LDA <entity_lo_y,X	; Get object Y
	ADD #16			; +16 as an approximate bottom (works for most ground troops)
	ADD <var2		; Add detection height
	STA <var2		; -> var2

	LDA <entity_hi_y,X	; Get object Y Hi
	ADC #0			; Apply carry
	STA <var1		; Object Y High offset -> var1

	LDA <player_lo_y		; Get Player Y
	SUB #22			; Subtract 22 (approximate Player height)
	STA <var4		; -> Temp_Var 4

	LDA <player_hi_y		; Get Player Y Hi
	SBC #0			; Apply carry
	STA <var3		; -> var3
...
PRG000_D297:
	STA entity_state,X	 ; Set appropriate object state
...
PRG000_D355:
	...
	ORA entity_no_collision_timer,X	; ... or this object's timer2 is not expired ...
...
PRG000_D373:
	STY <var1		 ; var1 = 0 (Player moving left) or 1 (Player moving right)

	EOR <entity_lo_x_velocity,X	 ; Check for X velocity sign difference between Player and object
...
PRG000_D399:
	STA entity_flipped_animation,X	 ; Update object flip bits as necessary
...
ObjState_Initializing:
	...
	INC entity_state,X	 	; Set object state to 2 (Normal run)
...
ObjState_Initializing_Call:
	...
	STA <var2
	JMP [var1]	 ; Dynamically jump to object's init routine 
...
PRG000_D3EF:
	...
	ADC <var1	 ; So value is now 0-5
	TAY		 ; -> 'Y'

	LDA <Horz_Scroll
	ADD PRG000_D3CC,Y	 ; Horizontal scroll plus offset

	ROL <var1		 ; var1 is 0/1, 4/5

	CMP <entity_lo_x,X	 ; Compare to object X
	PHP		 	; Save CPU state
...
PRG000_D449:
	...
	LDY entity_var6,X ; Y = Var6 (should be 0 or 1, the buffer slot the object occupies)
...
PRG000_D464:
	...
	CMP <entity_lo_y,X	 ; Compare to object Y
...
Object_DoNormal:
	...
	STA <var2
	JMP [var1]	 ; Dynamically jump to object's "normal" routine 
...
Object_DoHaltedAction:
	...
	.word exit_flag_draw			; 11: Bank2/End Level Card ONLY
...
; var1 = Object sprite Y
; var2 = Object sprite X
; var3 = Object's LR flag
; var4 = Object's attributes
; var5 = Objects_SprVVis
; var6 = Object's starting tiles index (and -> 'X')
; var7 = Object's Sprite_RAM offset (and -> 'Y')
...
Object_ShakeAndCalcSprite:
	...
	STA <entity_sprite_lo_y,X	; Store as sprite Y
	STA <var1		; Also -> var1
...
PRG000_D5A0:
	LDA <entity_lo_x,X	; Get object's X
	SBC <Horz_Scroll	; Make relative
	STA <entity_sprite_lo_x,X	; Store as sprite X
	STA <var2		; Also -> var2

	LDA entity_flipped_animation,X
	STA <var3		; var3 = object's LR flag
...
PRG000_D5BF:
	STA <var4		; Decided attributes -> var4

	LDA Objects_SprVVis,X
	STA <var5		; var5 = Objects_SprVVis

	LDA Objects_SprHVis,X
	STA <var8		; var8 = Objects_SprVVis

	; Each "frame" value moves up two bytes to the next pair
	LDA entity_animation_frame,X
	ASL A	
	STA <var6		; var6 = entity_animation_frame << 1

	LDA ObjectGroup_PatternStarts,Y
	ADD <var6	
	STA <var6		; var6 += ObjectGroup_PatternStarts[Y] (set base tile index)
...
Object_Draw16x32SpriteCS:	; Alternate entry point assuming Object_ShakeAndCalcSprite called
	...
	LDA <var3	 ; entity_flipped_animation
	BPL PRG000_D62D	 ; If entity_flipped_animation bit 7 is NOT set, jump to PRG000_D62D
...
PRG000_D62D:
	...
	LDA <var3	 ; entity_flipped_animation
	BMI PRG000_D63F	 ; If entity_flipped_animation bit 7 set, jump to PRG000_D63F
...
PRG000_D67B:
	TYA		 
	STA entity_animation_frame,X	 ; Set as frame 0 or 1
	RTS		 ; Return
...
; var8 = Objects_SprHVis
	...
	LDA <var1	; Get sprite Y

	BIT <var8	; Testing bit 7 of horizontal sprite visibility
...
PRG000_D693:
	...
	LDA <var3		 
	ORA <var4		 ; Joins base attributes to H-flip flag
...
;
; Returns:
;	var7 = offset to the blank sprite (also in 'Y')
;	var8 = 5 or 1
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
PRG000_D73A:
	...
	AND #$07	
	ADD <var7	
	TAY		 ; Y = (Random 0-7) + var7

	LDA PRG000_D727,Y
	TAY		
	STY <var9	 ; var9 = PRG000_D727[Y]
...
PRG000_D763:
	...
	LDA <var9
	CMP #$05	
	BLT PRG000_D776	 ; If var9 < 5, jump to PRG000_D776

	LDY #$01	 ; Y = 1

PRG000_D776:
	STY <var8	 ; var8 = 5 or 1
...
PRG000_D789:
	STX <var7	 ; var7 = offset to blank sprite
...
PRG000_D809:
	LDA <var2		
	SUB Level_VertScroll
	STA <var4		 ; var4 = screen-relative Y
...
PRG000_D82B:
	...
	; var1 - upper left of bounding box
	; var2 - offset to right bounding box (width)
	; var5 - top of bounding box
	; var6 - offset to bottom bounding box (height)
Player_BoundBox:
	;    Left Right  Bot Top - offsets applied to sprite X/Y
...
PRG000_D83D:
	STA <var16	 ; var16 = 0 or 1, depending on entry point
...
Player_BBox_NoRev:
	...
	STA <var1		 ; Store Player's bounding box top offset -> var1

	LDA Level_7Vertical
	BNE PRG000_D8B1	 ; If level is vertical, jump to PRG000_D8B1

	; Calculate full 16-bit X difference of object -> var14/15
	LDA <player_lo_x
	SUB <entity_lo_x,X
	STA <var15	
	...
	; Otherwise, 16-bit negate var14/15
	LDA <var15
	JSR Negate	
	STA <var15	
...
PRG000_D8A9:
	...
	LDA <var14
	BNE PRG000_D920	 ; If var14 is not zero, there's a difference in the "High" component of the Player/Object, so no intersect!  Jump to PRG000_D920

	LDA <var15
	BMI PRG000_D920	 ; If var15 is negative, no intersect, jump to PRG000_D920
...
PRG000_D8B1:
	...
	; Calculate full 16-bit Y difference of object -> var14/15
	LDA <player_lo_y
	SUB <entity_lo_y,X
	STA <var15	
...
PRG000_D8CF:
	...
	LDA <var1	 ; Get Player's bounding box top offset
	SUB <var11	 ; Get vertical difference between Player and Object's bounding box bottoms
	CMP #$08	 
	BLS PRG000_D8EB	 ; If the result < 8, jump to PRG000_D8EB

	; Otherwise, flip the remaining bit of var12
	LDA <var12	
	EOR #$01
	STA <var12	
...
PRG000_D8EB:
	...
	LDA #OBJSTATE_KILLED
	STA entity_state,X	 ; Set object state to Killed

	LDA #-$38	
	STA <entity_lo_y_velocity,X	 ; Set Y Velocity to -$38
...
PRG000_D922:
	...
	LDA <var16	 ; Returns 0 or 1, depending on original entry point
...
PRG000_D929:
	...
	; var1 - upper left of bounding box
	; var2 - offset to right bounding box
	; var5 - bottom of bounding box
	; var6 - offset to top bounding box
; $D92F
Object_CalcBoundBox:
	; X is object's slot
	; Y is group relative object index
	...
	STA <var1		 ; var1 object's sprite X with offset

	LDA <entity_sprite_lo_y,X
	ADD Object_BoundBox+2,Y	
	STA <var5		 ; var5 object's sprite Y with offset

	LDA Object_BoundBox+1,Y
	STA <var2		 ; var2 has the right offset

	LDA Object_BoundBox+3,Y	
	STA <var6		 ; var6 has the bottom offset
	...
	; Calculates the upper-left and returns the lower-right offsets
	; of this object's bounding box...
	; var3 - upper left of bounding box
	; var4 - offset to right bounding box
	; var7 - bottom of bounding box
	; var8 - offset to top bounding box
; $D955
Object_CalcBoundBox2:	; Same as Object_CalcBoundBox in spirit, just different outputs, used for object-to-object collision
	...
	LDA <entity_sprite_lo_x,X
	ADD Object_BoundBox,Y	
	STA <var3		 ; var3 object's sprite X with offset

	LDA <entity_sprite_lo_y,X
	ADD Object_BoundBox+2,Y	
	STA <var7		 ; var7 object's sprite Y with offset

	LDA Object_BoundBox+1,Y
	STA <var4		 ; var4 has the right offset

	LDA Object_BoundBox+3,Y	
	STA <var8		 ; var8 has the bottom offset
...
ObjectObject_Intersect:
	LDY #$00	 ; Y = 0
	STY <var12	 ; var12 = 0 (Player's bbox is to the RIGHT of the object's left edge, until determined otherwise)


	; Right now we're considering the left edges of the Player and Object
	; we're detecting against

	LDA <var3		 ; Left of Player's bounding box
	SUB <var1		 ; Left of object's bound box
	STA <var11		 ; Difference between the two -> var11

	ASL A			 ; Sets carry if it was a negative result
 
	LDA <var2		 ; Get the object's right edge bounding box offset
	...
	; Negate var11 (the difference), making it positive
	LDA <var11
	EOR #$ff
	STA <var11		 
	INC <var11	

	INC <var12		 ; var12 = 1 (Player's bbox is to the LEFT of object's left edge)

	LDA <var4		 ; Get the Player's right edge bounding box offset
...
PRG000_D997:
	...
	ASL <var12		 ; Push result bit up

	LDA <var7		 ; Player's bounding box bottom
	SUB <var5		 ; Object's bouncing box bottom
	STA <var11		 ; Store difference -> var11

	ASL A		 	 ; Sets carry if it was a negative result

	LDA <var6		 ; Get object's bounding box top edge offset
	...
	INC <var12		 ; Set bit 0 of var12

	LDA <var8		 ; Get Player's bounding box top edge offset

PRG000_D9B5:
	CMP <var11		 ; Set final carry bit to determine if we intersected or not
...
; $D9BB
Object_DoCollision:
	LDX <entity_index	; X = the current object slot
	...
	STA <var2	
	JMP [var1]	 ; Jump to the acquired address!
...
Player_GetHurt:
	...
	LDA <player_powerup
	CMP #powerup_fire		; SB: Change this to "powerup_super_start" and you restore Japanese version's "always shrink" code!!
...
Object_HitByTailOrBouncer:
	...
	STA <var7	 ; var7 = LRBounce_Y

	LDA #$0f
	STA <var8	 ; var8 = $0F

	LDA LRBounce_X
	STA <var3	 ; var3 = LRBounce_X
...
PRG000_DB2F:
	...
	ADD <player_sprite_lo_x
	STA <var3	 ; var3 = appropriate X offset for tail attack
	...
	LDA <player_sprite_lo_y
	ADD #$10
	STA <var7	 ; var7 = tail attack offset

	LDA #$0f
	STA <var8	 ; var8 = $0F

Player_TailAttackDo:
	LDY entity_state,X	; Y = object's current state
...
PRG000_DC0B:
	...
	LDA <entity_lo_x,X
	SUB entity_lo_x,Y		; Difference between the object and the OTHER object's X
	PHA		 	; Save the difference

	LDA <entity_hi_x,X
	SBC entity_hi_x,Y	 ; Difference between the object and the OTHER object's X Hi
	STA <var1		 ; -> var1
...
PRG000_DC57:
	...
	LDA entity_type,Y	 ; A = the ID of the OTHER object
...
PRG000_DC5C:
	...
	; This changes a tile based on var13-16 coordinates:
...
PRG000_DC7B:
	LDA <var15
	ASL A		
	TAY		 ; Y = var15 (X Hi) shifted left 1 (2 bytes index per screen for Tile_Mem_Addr)
	...
	INC <var2	 ; Otherwise, offset to lower screen area
...
PRG000_DC91:
	AND #$f0
	STA <var3	; Tile aligned "low" (makes row)
	...
	LSR A	
	LSR A	
	ORA <var3	; Makes column

	TAY		 ; Y = offset to specific tile

	PLA		 ; Restore 'A' (tile index)

	STA [var1],Y ; Change tile as appropriate
...
Object_ApplyXVel:
	...
	STA <entity_hi_x,X	; Otherwise, Object's X Hi is zeroed
...
PRG000_DD19:
	...
	ADC <entity_lo_x,X 
	STA <entity_lo_x,X ; Add with carry

	TYA		 ; A = $00 or $FF as appropriate

	ADC <entity_hi_x,X
	STA <entity_hi_x,X ; Add to the high part
...
; Returns: var16 as pixel difference between Player and object X coordinates
...
;
; For a given object slot in 'X'...
; Returns: var16 as pixel difference between Player and object Y coordinates
