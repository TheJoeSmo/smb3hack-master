...
; (^\s*.*\s*); (PRG005_[0-9A-F][0-9A-F][0-9A-F][0-9A-F])	-->	\n\2:\n_1	(Labels from comment to beginning of line)
; ^(\s*[BJ].. PRG005_[0-9A-F][0-9A-F][0-9A-F][0-9A-F]).*	-->	_1\n		(Formatting linebreak after B.. J..)
...
; the NMI does employ Temp_Var1-3, and restores them when it's done.
	...
	Thunk_Flags:	.dsb 1	; Temporary variable used to keep status flags after a Thunk call
...
; the palette data as bg / sprite colors.  FIXME is this used though??
	...
	Thunk_Bank:			.dsb 1	; Bank to switch to when making a thunk call
	...
	Thunk_Jump_L:		.dsb 1	; Low byte of address to perform thunk jump (JMP/JSR_THUNKA/C)
	Thunk_Jump_H:		.dsb 1	; High byte of address to perform thunk jump (JMP/JSR_THUNKA/C)
...
; different (especially during the montage) so consider the overlapped variables in the next section
	...
	; This might change...
	Title_XPosHi:		.dsb 3	; $75-$7C "High" part of the extended precision X position for all objects
	Title_YPosHi:		.dsb 3	; $7D-$84 "High" part of the extended precision X position for all objects
	Title_ObjX:		.dsb 3	; $85-$8C Title screen object X positions
	Title_ObjY:		.dsb 3	; $8D-$94 Title screen object Y positions
	Title_ObjXVel:		.dsb 3	; $95-$9C X velocities of title screen objects (4.4FP)
	Title_ObjYVel:		.dsb 3	; $9D-$A3 Y velocities of title screen objects 
	Title_XPosFrac:		.dsb 3	; $A5-$AC X position extended precision of objects (provides 4-bit fixed point)
	Title_YPosFrac:		.dsb 3	; $AD-$B4 Y position extended precision of objects (provides 4-bit fixed point)
	Title_ObjYVelChng:	.dsb 3	; $B5-$B6 Mario / Luigi change in Y velocity flag
	Title_ObjMLFlags:	.dsb 3	; $B7-$B8 Mario / Luigi Sprite flags
	Title_ObjMLMoveDir:	.dsb 3	; Used in ending for floating
	Title_ObjMLFrame:	.dsb 3	; $BF-$C0 Mario / Luigi next sprite to display
	Title_ObjMLPower:	.dsb 3	; $C1-$C2 Mario / Luigi current powerup (0 = Small, 1 = Big, 3 = Leaf)
	Title_ObjMLSprRAMOff:	.dsb 3	; $C3-$C4 Mario / Luigi Defines a Sprite_RAM offset for Mario / Luigi
	Title_ObjMLSprVis:	.dsb 3	; $C5-$C6 Mario / Luigi sprite sliver visibility bits (generated by Title_MLDetermineSpriteVis)
	Title_CurMLIndex:	.dsb 1	; 0 for Mario, 1 for Luigi
	
	Title_HorzScroll:	.dsb 1	; SB: Horizontal scrolling of ground
	...
	Title_InitSaveMCnt:	.dsb 1	; Counter used by initializing save menu for clearing the taglines and bringing up save slots
	...
	Title_CM_EvFlags:	.dsb 1	; Title screen config menu event flags:
	...
	Title_CheatIndex:	.dsb 1	; Debug cheat counter	
	Title_DebugCursor:	.dsb 1	; Debug cursor position
	Title_DebugSTest:	.dsb 1	; Debug menu sound test index
	Title_DebugComet:	.dsb 1	; Debug menu comet level index
	Title_ComplWorld:	.dsb 1	; Debug menu complete-up-to-world (effects Star Road unlock only)
	Title_WorldEnd:		.dsb 1	; Debug menu world end index; Triggers world end sequence for given world
...
; There's a consistent difference of $12 between X and Y; this consistent distancing is meant to be maintained, so leave it alone!
	...
	NMI_VertTemp:		.dsb 1	; SB: Temporary used when calculating vertical offsets in NMI
...
; NOTE!! This object var is OBJECT SLOT 0 - 4 ONLY!
	...
	Level_GndLUT_L:		.dsb 1
	Level_GndLUT_H:		.dsb 1
...
PLAYERSUIT_LAST		= PLAYERSUIT_HAMMER	; Marker for "last" suit (Debug cycler needs it)
	...
	Player_OldSuit:		.dsb 1	; Player's previous power-up, to support new transition effect
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	Level_WaterMuck:	.dsb 1	; If non-zero, water is muck-like!
	
	Level_TWCoolDown:	.dsb 1	; If non-zero, touch warp objects won't fire (decrements to zero)
...
UPDATERASTER_32PIXSHOWSPR= $80	; If NOT set, hides sprites that fall beneath the partition (i.e. for fixed water effect)
	...
	Level_JctYLHStart:	.dsb 16	; Array of Y / YHi starts
	Level_JctXLHStart:	.dsb 16	; Array of X / XHi starts

	Level_PipeRaiseData:	.dsb 1	; Bitfield for raising pipes 1-4; bits 0-3 are for each needing to be raised, bits 4-7 are each already raised

	Level_PauseSelect:		.dsb 1	; PAUSE Menu selection
...
; W8D = World 8 Darkness; overlaps the vars used by the entrance transition
	...
	Map_StarCoin_Got:	.dsb 1
	Map_StarCoin_Total:	.dsb 1

	Map_BorderAttrFromTiles:.dsb 64	; $7EBE-$7EC8 (?) Attributes collected from map tiles that get overwritten by border FIXME SIZE UNCERTAIN
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	Splash_Counter:		.dsb 3	; Water splash counter
	Splash_Y:		.dsb 3	; Water splash X
	Splash_X:		.dsb 3	; Water splash Y
	Splash_NoScrollY:	.dsb 3	; If set, flags this water splash to not display sprite Y as relative to screen scroll

	BrickBust_En:		.dsb 3	; Brick bust "Enable" (0 = disable, 2 = brick debris, anything else = "poof" away)
	BrickBust_YUpr:		.dsb 3	; Brick bust upper chunks Y
	BrickBust_X:		.dsb 3	; Brick bust base X
	BrickBust_YVel:		.dsb 3	; Brick bust Y velocity
	BrickBust_XDist:	.dsb 3	; Brick bust X split
	BrickBust_YLwr:		.dsb 3	; Brick bust lower chunks Y
	BrickBust_HEn:		.dsb 3	; Bits to hide chunks (Bit 0 = Right, 1 = Left, 2 = Lower, 3 = Upper) OR poof counter

	CoinPUp_State:		.dsb 4	; State of up to 4 "Power Up" coins (i.e. coins that come out of ? blocks and bricks)
	CoinPUp_Y:		.dsb 4	; Y of "Power Up" coins
	CoinPUp_X:		.dsb 4	; X of "Power Up" coins
	CoinPUp_YVel:		.dsb 4	; Y velocity of "Power Up" coins
	CoinPUp_Counter:	.dsb 4	; Counter used by "Power Up" coins
...
SOBJ_ALBABOMB	= $19	; Albatoss's bomb!
	SpecialObj_ID:		.dsb 8	; Special object spawn event IDs

	Objects_Var3:		.dsb 5	; Generic variable 3 for objects SLOT 0 - 4 ONLY

	SpecialObj_YHi:		.dsb 8	; Special object Y high coordinate

	Objects_LastTile:	.dsb 8	; Last tile this object detected

	Objects_SprAttr:	.dsb 8	; Object sprite attributes (only uses bit 6 for H-Flip and bits 0-1 for palette)
	Objects_IsBehind:	.dsb 8	; If set, object will use a short horizontal test to determine if it is off-screen

	Roulette_Lives:			; Number of lives you are rewarded from winning the Roulette (NOTE: Shared with first byte of Objects_IsGiant)
	Objects_IsGiant:	.dsb 8	; Set mainly for World 4 "Giant" enemies (but some others, like Bowser, also use it)
	...
	Level_OnOff:		.dsb 1 	; On Off
	THouse_OpenByID:	.dsb 15	; UNUSED would keep track of chests opened for a given Toad House ID (THouse_ID)
	StatusBar_PMT:		.dsb 8	; Status bar tiles that currently make up the power meter >>>>>>[P]
	...
	StatusBar_Score:	.dsb 6	; Status bar tiles for score
	StatusBar_Time:		.dsb 3	; Status bar tiles for time remaining
	...
	Bubble_Cnt:		.dsb 3	; Bubble counter value (0 = no bubble)

	Bubble_YHi:		.dsb 3	; Water Bubble Y Hi
	Bubble_Y:		.dsb 3	; Water Bubble Y
	Bubble_XHi:		.dsb 3	; Water Bubble X Hi
	Bubble_X:		.dsb 3	; Water Bubble X

	SpecialObj_VisFlags:	.dsb 8	; Special object visibility flags (Bit 0 = Set if vertically off-screen, Bits 7-6 set when each 8x16 sprite is horizontally off-screen (left-to-right from MSb))
	
	Level_PCometCoins:	.dsb 2	; 16-bit counter of purple comet coins remaining

	Player_DebugNoHitFlag:	.dsb 1	; DEBUG: When set, disables getting hurt
...
; Lower ranges are context-dependent
	...
	Music_MusBank:		.dsb 1	; SB: New -- sets the current "music bank" for Set 2B/2C music (default 31, but grow, music, grow!)

	Music_InvertEn:		.dsb 1	; Enables the inverted music scale (NOTE: Use value of 0 to disable or 24/$18 to enable!!)
	Music_Invert:		.dsb 1	; Inverting is active (so we can clear it for sound effects) (Will be 0 or 24/$18)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	.base $05F0
	Ending_TextPos:		.dsb 1	; Ending text position
	Ending_VAddrL:		.dsb 1	; Ending text DBYT low
	Ending_VAddrH:		.dsb 1	; Ending text DBYT high
	Ending_LineNum:		.dsb 1	; Line number (tracks line breaks)
	Ending2_World:		.dsb 1	; Current world of Ending 2
	Ending2_FadeLevel:	.dsb 1	; Fade out 0, 1, 2, 3, 4
	Ending2_State:		.dsb 1	; Ending 2 state
	Ending2_Timer:		.dsb 1	; Counts to zero
	Ending2_SpawnIndex:	.dsb 1	; Enemy spawn index
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	Map_Objects_Y:		.dsb 9	; $7EEB-$7EF8, Y coordinate of all map objects
	Map_Objects_XLo:	.dsb 9	; $7EF9-$7F06, X coordinate lo byte of all map objects
	Map_Objects_XHi:	.dsb 9	; $7F07-$7F14, X coordinate hi byte of all map objects
...
MAPOBJ_TOTAL		= $09	; Total POSSIBLE map objects
	Map_Objects_IDs:	.dsb 9	; $7F15-$7F22
	Map_Objects_Itm:	.dsb 9	; $7956-$795D, "Item given by" map objects
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	Player_PenguinHopCnt:	.dsb 1	; Counter used for frog hopping along the ground (shared with Player_FireCount)

	Obj2Obj_FlagNoHitChk:.dsb 1	; SB: One-shot flag that skips the OAT_HITNOTKILL check in ObjectToObject_HitTest
	...
	Objects_Timer2:		.dsb 8	; $0520-$0525 "Timer" values; automatically decrements to zero 

	Objects_FrozenTimer:.dsb 8	; SB: Ticks remaining for frozen state (except $FF = kicked and no decrement)
	...
	Player_OnStairsX:	.dsb 1	; Player's last X position on stairs (only tracks 8-bit differences)
	Player_OnStairs:	.dsb 1	; Flags; $80 means on stairs at all, $02 means on leftward stairs
	Player_DisTailAtk:	.dsb 1	; If set, blocks tail attack / weapon firing (cleared when handled)
...
ACTSW_REVERSEGRAV	= 10	; Reverse gravity
	Level_ActSwEvent:	.dsb 1	; Action Switch action to be performed when such a switch is pressed
	Level_ActSwYHi:		.dsb 1	; Y Hi of action switch that was most recently hit
	Level_ActSwY:		.dsb 1	; Y of action switch that was most recently hit
	Level_ActSwXHi:		.dsb 1	; X Hi of action switch that was most recently hit
	Level_ActSwX:		.dsb 1	; X of action switch that was most recently hit
	Level_ActSwAction:	.dsb 1	; Set to Level_ActSwEvent when switch pressed (thus 0 = disabled)
	Level_ActSwState:	.dsb 1	; State of action switch action
	Level_ActSwVar1:	.dsb 1	; Action switch arbitrary variable 1
	Level_ActSwVar2:	.dsb 1	; Action switch arbitrary variable 2
	Level_ActSwVar3:	.dsb 1	; Action switch arbitrary variable 3 (initially set to tile that was pressed)
	...
	Level_32PPSlopeFlag:.dsb 1	; FIUX
...
CHNGTILE_SANDTOPMID = $33	; Quick sand stop and middle tile
	...
	Level_ChgTileEvent:	.dsb 1	; When non-zero, queues a "change tile" event (SB: New feature, set bit 7 to update attributes too; single tile change only)
	...
	Player_ClimbJumpUpFlag:.dsb 1	; Set after player jumps while climbing and holding up; reset as soon as they're falling or let go of Up

	Player_SlideRate:	.dsb 1	; While Player is sliding, this is added to X Velocity (does not persist, however)

	Level_JctCtl_Req:	.dsb 1	; Used by Touch Warp to request junction when appropriate to execute (special object can't do it)
	...
	Player_Kuribo:		.dsb 1	; SB: When > 0, riding Yoshi; value specifies the object slot
	...
	Player_Statue:		.dsb 1	; Player is in Rabbit Statue mode; counts down to zero
	...
	Player_Behind:		.dsb 1	; When non-zero, Player is "behind the scenes" SB: Now a permanent flag, not a countdown
...
ASCONFIG_HDISABLE	= $80	; Disables horizontal auto scroll coordinate adjustment (generally if Horz Auto Scroll is not in use)
	...
	Player_GravityEffect:	.dsb 1	; 0 = Normal, 1 = Low Gravity
	
	PlayerProj_YVelFrac:	.dsb 2
	PlayerProj_Var2:		.dsb 2
	PlayerProj_XHi:			.dsb 2

	Player_DigSand:			.dsb 1	; Player sand digging; counts to zero
...
;	In all other states, timer decrements every 2 ticks
	...
	Objects_InWater:	.dsb 8	; $06B7-$06BB Set when object is in water
	...
	Player_ReverseGrav:	.dsb 1	; When set, Player reverse gravity (due to many, many dependencies, expecting exactly $01 to set reverse gravity)
	Objects_ReverseGrav:	.dsb 8	; $06F4-$06FB Object with reverse gravity (SB)
	...
	Level_AltLayout:	.dsb 2	; Pointer to level's "alternate" layout (when you go into bonus pipe, etc.)
	Level_AltObjects:	.dsb 2	; Pointer to level's "alternate" object set (when you go into bonus pipe, etc.)

	Level_AltLayBackup:	.dsb 2	; Alternate level layout address BACKUP used for OBJ_ALTLEVELMOD
	Level_AltObjBackup:	.dsb 2	; Alternate level object address BACKUP used for OBJ_ALTLEVELMOD
	Level_AltTSBackup:	.dsb 1	; Alternate tileset BACKUP used for OBJ_ALTLEVELMOD

	Objects_StompDisable:	.dsb 8	; Disable stomping of object for one frame
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	Player_Character:	.dsb 2	; Who each Player is playing as (0 = Mario, 1 = Luigi, 2+ = Toad)
	...
	Bonus_DDDD:		.dsb 1	; ?? Set to '1' if you exit the unused bonus game BONUS_ARENA_KEY2
	Map_HandState:		.dsb 1	; Hand trap state

	Map_UpdStarCoinsCnt:	.dsb 1	; SB: Counter (from 3 downward) to update one star coin at a time (prevent full-screen flicker)
	...
	Player_FallToKing:	.dsb 1	; Player will fall to king when Level_ExitToMap is nonzero (instead of map)
						.dsb 1	; unused
	...
	Map_WasAltExit:		.dsb 1	; SB: If set, this was an alternate exit

	Map_DoFortressFX:	.dsb 1	; SB: Kind of leftover, dunno if this will be revitalized??

	World_Map_Power:	.dsb 2	; $0746-$0747 (Mario/Luigi) MAP Power up # (1 - big, 2 - Fire, 3 - Leaf, 4 - Penguin, 5 - Rabbit, 6 - Hammer, 7 - Judgems, 8 - Pwing, 9 - Star)
	...
	Object_XVelCarry:	.dsb 1	; '1' when last Object X Velocity fraction accumulation rolled over (SB: It's specific now)
	...
	THouse_UnusedFlag:	.dsb 1	; Cleared when Toad House gives you an item, but never used otherwise
				...
				.dsb 6	; $0758-$075E unused

	; WARNING: The distance between Player/Objects_XVelFrac and Player/Objects_YVelFrac must be same as Player/Objects_X/YVel!
	Object_YVelCarry:	.dsb 1	; '1' when last Object Y Velocity fraction accumulation rolled over (SB: It's specific now)
...
RandomN = Random_Pool+1			; Pull a random number from the sequence (NOTE: RandomN+1 is also good; If you need multiple random numbers, call Randomize)
	...
	Level_IsComplete:	.dsb 1	; Set upon entering a level indicating it was already completed somehow (allows START+SELECT exit and other things)
...
; ********************************************************************************
	...
	Sound_Sq1_CurFL:	.dsb 1	; Holds current "low" frequency of Square Wave 1 (Warning: Must be +4 to Sound_Sq2_CurFL, see PRG063_E808)
	...
	Sound_Sq2_CurFL:	.dsb 1	; Holds current "low" frequency of Square Wave 1 (Warning: Must be +4 from Sound_Sq1_CurFL, see PRG063_E808)
...
Tile_Mem:	.dsb 6480	; $6000-$794F Space used to store the 16x16 "tiles" that make up the World Map or Level
	...
	;LevCP_BaseID:		.dsb 1	; Base ID for this world	<-- Only need this if persisting across worlds
	LevCP_ActiveID:		.dsb 1	; Active level's checkpoint ID

	; Player 1's checkpoint
	LevCP_ID:			.dsb 1	; ID of checkpoint (must match LevCP_ActiveID to be used)
	LevCP_LayBackup:	.dsb 2	; Layout pointer backup as set by checkpoint
	LevCP_ObjBackup:	.dsb 2	; Object pointer backup as set by checkpoint
	LevCP_TSSCBackup:	.dsb 1	; Tileset (lower 4 bits) and star coins (upper 4 bits) backup as set by checkpoint
	LevCP_X:			.dsb 1	; Player X / Horizontal scroll low (H-Scroll set as this value - $80)
	LevCP_XHi:			.dsb 1	; Hi component of horizontal (-> PXhi and ->SHi)
	LevCP_Y:			.dsb 1	; Player Y
	LevCP_YHi:			.dsb 1	; Hi component of Player Y
LevCP_End

	; Player 2's checkpoint
	LevCP_ID2:			.dsb 1	; ID of checkpoint (must match LevCP_ActiveID to be used)
	LevCP_LayBackup2:	.dsb 2	; Layout pointer backup as set by checkpoint
	LevCP_ObjBackup2:	.dsb 2	; Object pointer backup as set by checkpoint
	LevCP_TSSCBackup2:	.dsb 1	; Tileset (lower 4 bits) and star coins (upper 4 bits) backup as set by checkpoint
	LevCP_X2:			.dsb 1	; Player X / Horizontal scroll low (H-Scroll set as this value - $80)
	LevCP_XHi2:			.dsb 1	; Hi component of horizontal (-> PXhi and ->SHi)
	LevCP_Y2:			.dsb 1	; Player Y
	LevCP_YHi2:			.dsb 1	; Hi component of Player Y
	...
	Map_SetBothPlayers:	.dsb 1	; If set, initialize BOTH Players to start position on map (only for NEW games)
	...
	Map_Previous_World:	.dsb 2	; $797C-$797D (Mario/Luigi) World Player is on (SB)
	...
	Map_WVisit1:		.dsb 1	; Map world visited bit flag 1 (World 1-8)
	Map_WVisit2:		.dsb 1	; Map world visited bit flag 2 (World 0)
	...
	Level_AScrlPosHHi:	.dsb 1	; Raster effect horizontal "high" position
		...
		.dsb 1	; Unused (warning: Cleared to $80 by EOW)
...
; Player's hammer/fireball
	...
	Horz_Scroll_Off32PP:.dsb 1	; Horizontal offset; currently only for 32 pixel partition mode (Big Boo needs this)
...
; Some variables used by the recovered magic wand
	...
	Debug_ForceStarCoins: .dsb 1	; When set, changes Map_CountStarCoins to actually give you the star coins
	
	Level_ArenaData:	.dsb 1	; Arena data; each bit is set per completed boss
	...
	; C = Warp Whistle
	; D = Music Box
	Inventory_Items:	.dsb 9	; Inventory, common both Players, 9 items
	...
	Map_ObjBackup_Y:	.dsb 9*2	; $7EEB-$7EF8, Y coordinate of all map objects
	Map_ObjBackup_XLo:	.dsb 9*2	; $7EF9-$7F06, X coordinate lo byte of all map objects
	Map_ObjBackup_XHi:	.dsb 9*2	; $7F07-$7F14, X coordinate hi byte of all map objects
	Map_ObjBackup_IDs:	.dsb 9*2	; $7F15-$7F22
	Map_ObjBackup_Itm:	.dsb 9*2	; $7956-$795D, "Item given by" map objects
	...
	Map_ObjBackup_World:	.dsb 2	; Object backup world sets
...
MCOMP_SECRET	= %00100000	; Bit 5 - Level Completed (secret/alternate exit)
	Map_Completions: .dsb 128	; SB 4/16/14 - I count $73 links, leaving a little buffer

	; Holds 8 bits (lining up to the first 8 map objects) per world
	Map_ObjCompletions:	.dsb 10	; NOTE: No map objects in Star Road or World Zero
	Map_CometMode:		.dsb 1	; Flag: When set, comets are active instead of regular map objects
	SaveData_Index:		.dsb 1	; Active save slot
